...
module Math_Operations
   ...
   !
   ! For 9x1 vectors and 9x9 matrices the tensorial index pairs `(1, 1), (2, 2), (3, 3), (1, 2), (2, 1), (2, 3), (3, 2), (1, 3), (3, 1)` in both dimensions are used
   real(dp), parameter, dimension(9) :: const_identity2d = [ &       ! 2dim identity matrix `\mathbf{I}` where `I_{ij} = \delta_{ij}`
      1.0_dp, 1.0_dp, 1.0_dp, &                                      ! with Kronecker-symbol `\delta_{ij} = 1` for `i=j` and 0 else
   ...
   real(dp), parameter, dimension(9, 9) :: const_identity4d_sym = reshape([ &
   ...
   real(dp), parameter, dimension(9, 9) :: const_identity4d_tr = reshape([ &
